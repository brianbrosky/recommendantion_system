...
@app.get("/recommendations/{advertiser_id}/{table_name}")
async def run_query_diaria(advertiser_id: str, table_name: str):
    # Definimos fecha
    fecha = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')
    
    # Construimos query
    sql_query = f"SELECT * FROM {table_name} WHERE advertiser_id = '{advertiser_id}' AND model_date = '{fecha}'"
...
async def run_query_7_días(advertiser_id: str):
    # Definimos periodo
    fecha_inicio = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')
    fecha_final = (datetime.now() - timedelta(days=7)).strftime('%Y-%m-%d')
    
    # Construimos queries
    top_20_query = f"SELECT * FROM top_20 WHERE advertiser_id = '{advertiser_id}' AND model_date  BETWEEN '{fecha_inicio}' AND '{fecha_final}'"
    top_20_ctr_query = f"SELECT * FROM top_20_ctr WHERE advertiser_id = '{advertiser_id}' AND model_date  BETWEEN '{fecha_inicio}' AND '{fecha_final}'"
